...
function SimParams = digitalBeamformerDesignV(SimParams)
...
switch precType{1}
    ...
    case 'ZF'
        ...
        for iRun = 1:SimParams.nMontRuns
            ...
            for iGroup = 1:SimParams.nGroups
                SimParams.groupInfo(iGroup).userChannel =0  zeros(SimParams.nReceive,SimParams.nTransmit,SimParams.groupInfo(iGroup).nUsers);
    ...
    case 'CVX'
        ...
        for iRun = 1:SimParams.nMontRuns
            ...
            barP = zeros(SimParams.nUsers,1);
            barQ = zeros(SimParams.nUsers,1);            
            for iGroup = 1:SimParams.nGroups
                for iUser = 1:SimParams.groupInfo(iGroup).nUsers
                    cUserIndex = SimParams.groupInfo(iGroup).gUserIndices(1,iUser);
                    barP(cUserIndex,1) = real(SimParams.groupInfo(iGroup).userChannel(:,:,iUser) * sBeamM * cvxInnerMP(:,cUserIndex));
                    barQ(cUserIndex,1) = imag(SimParams.groupInfo(iGroup).userChannel(:,:,iUser) * sBeamM * cvxInnerMP(:,cUserIndex));
                end
            end
            ...
            for iSca = 1:nSCAIterations
                ...
                expressions P(SimParams.nUsers,1) Q(SimParams.nUsers,1)
                ...
                for iGroup = 1:SimParams.nGroups
                    ...
                    for iUser = 1:SimParams.groupInfo(iGroup).nUsers
                        ...
                        P(cUserIndex,1) = real(effChannel * cvxInnerMP(:,cUserIndex));
                        Q(cUserIndex,1) = imag(effChannel * cvxInnerMP(:,cUserIndex));
                        
                        (barP(cUserIndex,1)^2 + barQ(cUserIndex,1)^2) / userBetaP(cUserIndex) ...
                            + 2 * (barP(cUserIndex,1) / userBetaP(cUserIndex,1)) * (P(cUserIndex,1) - barP(cUserIndex,1)) ...
                            + 2 * (barQ(cUserIndex,1) / userBetaP(cUserIndex,1)) * (Q(cUserIndex,1) - barQ(cUserIndex,1)) ...
                            - ((barP(cUserIndex,1)^2 + barQ(cUserIndex,1)^2) / userBetaP(cUserIndex)^2) * (userBeta(cUserIndex,1) - userBetaP(cUserIndex,1))...
                            == userRate(cUserIndex,1) - 1;
                ...
                if contains(cvx_status,'Solved')
                    ...
                    userBetaP = rand(SimParams.nUsers,1);
                    cvxInnerMP = complex(randn(SimParams.tStatBeams,SimParams.nUsers),randn(SimParams.tStatBeams,SimParams.nUsers));
                end
...
SimParams = evaluateUserRatesWithGroupPrecoders(SimParams,cvxInnerMP,precType{1});
fprintf('Final Rate - %s is : %d \n',precType{1},sum(SimParams.totUserRateE));
