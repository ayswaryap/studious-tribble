...
function legendName = plotFolderResultsKKT(varargin)
...
for iBeam = 1:length(beamFiles)
    ...
    %validFields = logical(arrayfun(@(x)(SimParams.groupSumRate.isSucceded(x)),1:SimParams.interIterations));
    ...
    userSumRate = max(SimParams.groupSumRate.srate(:,:),[],2);
    beamPlotY(1,iBeam) = mean(userSumRate);
    
    fprintf('Number of valid samples : %d for :: %s \n',SimParams.groupSumRate.srate,legendName{1,legendIndex});%sum(validFields),legendName{1,legendIndex});
