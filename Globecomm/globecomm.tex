...
%		\end{figure}
	...
	\begin{eqarray}{rCL}{group_specific}
		\underset{\gamma_{k}, b_{k}, \mathbf{w}_{k}} {\text{maximize}}  & \quad &  \sum_{k \in \mc{U}_g} \alpha_k \log({1 + \gamma_{k}})  \nonumber \\
		\text{subject to} &\quad& a_k: \eqref{icassp10} \geq \gamma_k\\
		&& b_k: \sum_{\mathclap{i \in \mathcal{U}_{g}\backslash \{k\} }} | \mathbf{h}_{k}^{\herm} \mathbf{B}_g \mathbf{w}_{i} |^2 + \sum_{\mathclap{\bar{g} \in {\mathcal{{G}}} \backslash \{g\}}} \bar{\zeta}_{\bar{g},k} + N_0 \leq b_{k} \eqspace \label{group_specific_1} \\
		&&c_k: \sum_{\substack{k \in \mathcal{U}_{{g}}}} | \mathbf{h}_{i}^{\herm} \mathbf{B}_{{g}} \mathbf{w}_{k} |^2 \leq  \bar{\zeta}_{{g},i}, \; \forall i \in \mathcal{U} \backslash {\mc{U}_g} \eqspace\ \label{group_specific_2} \\
		&&d_k: \sum_{g \in \mathcal{G}}\sum_{k \in \mathcal{U}_g} \|\mathbf{B}_g\mathbf{w}_{k} \|^2 \leq \frac{P_{\mrm{tot}}}{G},  \eqspace 
	\end{eqarray}
