...
%		\end{figure}
	...
	\begin{eqarray}{rCL}{update}
		\frac{\nabla L} {\nabla \gamma_k} \Rightarrow a_k^{(i)} &=& \frac{\alpha_k}{(1+ \gamma_k^{(i-1)})}\\
		\frac{\nabla L}{\nabla \beta_k} \Rightarrow b_k^{(i)} &=& \frac{| \mathbf{h}_{k}^{\herm} \mathbf{B}_g \mathbf{w}_{k} |^2}{\bar{\beta}_k} a_k^{(i)} \\
		\frac{\nabla L}{\nabla \zeta_{g,k}} \Rightarrow c_k^{(i)} &=& b_k^{(i)} \\
		\frac{\delta L}{\delta \mbf{w}_k} \Rightarrow \mbf{w}_k^{(i)} &=& \frac{a_k^{(i)}}{2} \big( \sum_{i \in \mc{U}_g , \backslash k}{ b_i^{(i)}} \mathbf{h}_{i}^{\herm} \mathbf{B}_g \mathbf{B}_g^{\herm} \mathbf{h}_{i} \nonumber \\ &+& \sum_{j \in \bar{g}} c_j^{(i)} \mathbf{h}_{j}^{\herm} \mathbf{B}_g \mathbf{B}_g^{\herm} \mathbf{h}_{j} + d_k I_{N_T}\big)^{-1} \mathbf{h}_{k}^{\herm}  \eqspace \\
